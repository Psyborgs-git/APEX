...
- **Copilot is a docked widget, NOT a tab** (cfa5a73+): `copilot` removed from VALID_TABS. Collapsed `Copilot` pill `copilot-dock-toggle` sits bottom-right ~tool(975,724) above the status bar → opens `copilot-dock-panel` anchored bottom-right (exactly 400×560, x1184 y220). Header minimize `copilot-dock-minimize` (~982,389) + close `copilot-dock-close` (~998,389) both collapse to pill; panel stays mounted (history preserved). `:COPILOT`/`:AI` commands open it via `copilotOpen`. Tool-trace chips `copilot-tool-trace` render only when tools were actually called (a direct quote reply from context shows none — use a backtest/build message to force the agentic loop). Dock floats over whatever tab is active.
...
- **Settings Save** (was broken d5af093, FIXED c8b38d5): `ensure_table` now probes `doc.get(section)` (was immutable `Index` panic `index not found` on missing tables). If a build still hangs "Saving…" with nothing persisting, the panic regressed — check backend log for `settings.rs` panics. Note provider/appearance writes don't refresh `AppState.llm` — changes apply on next backend start.
